...
template = codecs.open("template.json", "r", "utf-8").read()

frameP =  re.compile('<frame>([\s\S]+?)<frame_end>', re.S)
framePropP = re.compile('[\s]*([\d]+)[\s]+([\w]+)[\s\\t]')

frames = frameP.findall(template)
for frame in frames:
    print("----------frame------------")
    props = framePropP.findall(frame)
    for prop in props:
            print("---prop---")
            print(prop)
